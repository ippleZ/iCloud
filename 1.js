...
var para /(httphttps)\:\/\//.test(link0) ? link0 : content0.split("\n")[0];
var para1 para.slice(para.indexOf("#") 1).replace(/\type/g,"node_type_para_prefix").replace(/\emoji/g,"node_emoji_flag_prefix").replace(/\tag/g,"node_tag_prefix").replace(/\index/g,"node_index_prefix") //防止参数中其它位置也存在"#"
...
const ADDRes quantumult-x:///add-resource?remote-resourceurl-encoded-json
var RLink0 {
 "filter_remote": [],
 "rewrite_remote": [],
 "server_remote": [],
...
var Pflowmark0 && para1.indexOf("flow") ! -1 ? para1.split("flow")[1].split("&")[0] : 0; // 流量时间等参数
...
var Pobfs mark0 && para1.indexOf("obfs") ! -1 && version 770? para1.split("obfs")[1].split("&")[0] : ""; // 指定特殊情况下的 obfsxx-http 类型
...
// URL-Scheme 增加配置
var ADDres quantumult-x:///add-resource?remote-resourceurl-encoded-json
var RLink {
 "server_remote": [
  sremoteposition
 ],
...
function VCheck(cnt) {
 cntscnt.split("\n").filter(Boolean).map(itemitem.trim()).filter(item /http/.test(item)).map(item"\""item"\"")
 cntscnts.join(",\n")
 //console.log(cnts)
 return cnts
 }
...
function Profile_Handle() {
 let a content0
 PProfile PProfile1? "001":PProfile
 PProfile PProfile8? "010": PProfile
 PProfile PProfile9? "011": PProfile
 ...
 RLinkRLink.replace("sremoteposition",srm).replace("fremoteposition",frm).replace("rremoteposition",rrm)
 ADDresADDres.replace("url-encoded-json",encodeURIComponent(RLink))
}
...
if (Pflow!0) {
 Pflow Pflow.split(":")
 var BdateDate.parse(new Date(Pflow[0]))/1000
 var BtotalPflow[1]? Pflow[1]*1024*1024*1024 : 0
 var BusedPflow[2]? Pflow[2]*1024*1024*1024 : 0
 var BremainBtotal !0 ? Btotal-Bused : 1
...
var type0""
//flag1,2,3分别为 server、rewrite、rule 类型
var flag 1
...
function Parser() {
 ...
 if (type0 ! "web" && type0 ! "wrong-field" && type0 ! "JS-0"){
  ...
  if (version 670 && typec!"") { //尝试跳转到正确类型
   RLink0[Field[typec]].push(resource.link", opt-parsertrue, tag下次添加资源️长点️8⃣️") // 跳转URI-Scheme
...
function ParseUnknown(cnt){
 try {
  ...
  if (!/errorblockinvalidsupport/.test(cnt.toLowerCase())) {
...
function ResourceParse() {
 ...
 if (type0 "Subs-B64Encode") { // subs2QX 负责所有节点的转换
  ...
  //notify("subs","",content0Pudp0Ptfo0Pcert0PTls13)
  ...
  flag 3;
  total Rule_Handle(content0.split("\n").map(itemitem.trim()).filter(Boolean), Pout0, Pin0).filter(Boolean);
  ...
  if (Ppolicyset) {total policy_sets(total, Ppolicyset)}
   // filter 重复检测
  total total.length100? total.filter( (ele,pos)total.indexOf(ele) pos) : total
  total total.join("\n")
 } else if (content0.trim() "") {
  notify("‼️ 引用" "⟦" subtag "⟧" " 返回內容为空", "⁉️ 点通知跳转以确认链接是否失效", para.split("#")[0], nan_link);
  flag 0;
 } else if (type0 "sub-http") {
  let url VCheck(String(Base64.decode(content0.split("sub://")[1].split("#")[0])", opt-parsertrue, tag"(new Date()).getTime()))
 ...
 if (flag 1) { //server 类型统一处理
  ...
  if (Prname) {
   Prn Prname;
   total total.map(Rename);
   if(Pdbg1) {notify("rename","content",total)}
  }
  //2023-07-10 调整emoji操作顺序
  if (Pemoji) { total emoji_handle(total, Pemoji); }
  if (total.length 0){
   if (Psuffix1 Psuffix-1) {total Psuffix 1? total.map(type_suffix):total.map(type_prefix)
   }
   ...
   if(Perror 0) {
   notify(" 友情提示 " "⟦" subtag "⟧", "️️ 解析后无有效内容", " 请自行检查相关参数, 或者点击通知跳转并发送链接反馈", bug_link)
  }
   total errornode
...
function Type_Check(subs) {
  ...
  if ( (subs.indexOf(html) ! -1 subs.indexOf("doctype html") ! -1) && link0.indexOf("github.com" -1)) {
  ...
  } else if ( (((ModuleK.some(RewriteCheck) para1.indexOf("dstrewrite") ! -1) && (para1.indexOf("dstfilter") -1) && subs.indexOf("[Proxy]") -1) typeU "module") && typeU ! "nodes" && typeU ! "rule" && typeQ !"filter") { // Surge 类型 module /rule-set(含url-regex) 类型
   typec"rewrite"
   type (typeQ "unsupported" typeQ "rewrite")? "sgmodule" : "wrong-field"
  } else if ((/(hostname\nhostname)\s*\/.test(subi) RewriteK.some(RewriteCheck)) && para1.indexOf("dstfilter")-1 && subi.indexOf("securehostname") -1 && !/modulenodesrule/.test(typeU) && !(RuleK.some(RuleCheck) && typeQ "filter") && !(typeQ! "rewrite" && QXProfile.some(ProfileCheck))) {
   // 2022-07-20 remove constrain && !/\[(Proxyfilter_local)\]/.test(subs)
   typec "rewrite"
   type (typeQ "unsupported" typeQ "rewrite")? "rewrite":"wrong-field" //Quantumult X 类型 rewrite/ Surge Script/
  } else if (((RuleK.some(RuleCheck) && subs.indexOf(html) -1 ) typeU "rule" para1.indexOf("dstfilter")!-1) && typeU ! "nodes" && !(typeQ "server" && (QuanXK.some(NodeCheck) SurgeK.some(NodeCheck))) ) {
   // rule/filter类型
   // 2022-07-20 remove constrain && !/\[(Proxyserver_local)\]/.test(subs) adter html
   typec "filter"
   ...
   type (typeQ "unsupported" typeQ "filter")? "Rule":"wrong-field";
  } else if (subsn.length 1 && SubK2.some(NodeCheck2) && !/\[(Proxyfilter_local)\]/.test(subs)) { //未b64加密的多行URI 组合订阅
   typec "server-uri"
   type (typeQ "unsupported" typeQ "server" typeQ "uri") ? "Subs":"wrong-field"
  } else if ((subi.indexOf("tag") ! -1 && QuanXK.some(NodeCheck) && !/\[(Proxyfilter_local)\]/.test(subs)) typeU "list") {
   typec "server-quanx"
  ...
  } else if ((SurgeK.some(NodeCheck) && !/\[(Proxyfilter_local)\]/.test(subs)) typeU "list") {
   typec"server-surge"
   ...
   Perror 1 ; // 无需反馈
   notify("️ 你导入的链接内容为 JS 脚本"," 脚本内未有重写规则，无法解析使用", " 请️不要️跑来解析器️反馈 \n"link0)
   type "JS-0"
  } else if (typeQ "server" && subs.length100) { // 一些未知的b64 encode server case
...
function TagCheck_QX(content) {
 ...
 console.log(content)
  ...
  for (var i 0; i Olist.length; i) {
    ...
    if (item.replace(/ /gm, "").indexOf("tag") ! -1) {
      ...
      while (nmlist.indexOf(nm) ! -1) { //重名情形
       //notify("重名",nm,nmlist)
        nm ni0? nm NoReplace(ni1):nm.split(" ").slice(0,nm.split(" ").length-2).join(" ") NoReplace(ni1)
  ...
  if (nulllist.length 1) {
    ...
    notify("️ 引用" "⟦" subtag "⟧" " 内有" no "个空节点名 ", " 已将节点“类型IP”设为节点名", " " nulllist.join("\n "), nan_link)
  }
  if (duplist.length 1) {
    no duplist.length 10 ? emojino[duplist.length] : duplist.length;
   if (Pdel!1 && Pntf0 ! 0){
    notify("️ 引用" "⟦" subtag "⟧" " 内有" no "个名字重复的节点 ", " 已添加数字区分, 删除请添加参数 del1:", " " duplist.join("\n "), nan_link)
   } else if (Pdel 1 && Pntf0 ! 0) {
    notify("️ 引用" "⟦" subtag "⟧" " 内有" no "个名字重复的节点 ", "️ 已全部删除，如需保留请去除参数 del1:", " " duplist.join("\n "), nan_link)
   }
  }
  return Nlist
}
...
function index_handle(item) {
 items item.map(itemitem.trim()).filter(Boolean)
 let bArray.from(new Array(items.length),(val,index)index1);
 //console.log(b[0])
 for (var i0; i items.length;i){
  ...
  ind !/(012345678)/.test(ind) ? 0 : ind
  console.log("handle index"ind)
  items[i] items[i].replace(/node_index_prefix(\d{0,1})/g,PatternN((i1).toString(),"",ind))
 }
 }
 console.log(items)
 return items
...
function ToRaw(cnt) {
 ...
 if (cnt) {
  notify( "️️ 将尝试解析该资源" "⟦" subtag "⟧" , " 请正确使用GitHub的 raw 链接" , " 你的链接："link0"\n 正确链接："rawlink, {"open-url":rawlink})
 } else if(content0.indexOf("gridcell")!-1) {
  notify( "️️ 解析该资源" " ⟦" subtag "⟧ 失败" , " 你的链接似乎是目录，而不是文件" , " 你的链接："link0, {"open-url":link0})
 }
 return cnt
}
...
function OBFS_Handle(cnt,pobfs) {
 ...
 if (/obfs\s*\\s*http/.test(cnt) && pobfs!"invalid") { //只替换有 obfshttp 参数
...
function Session_Handle(cnt,psession) {
 let st ", tls-no-session-tickettrue"
 let sr ", tls-no-session-reusetrue"
 if (psession 0) {
  cnt cnt st
 } else if (psession 1) {
   cnt cnt sr
...
function SHA256_Handle(cnt,pcsha256,ppsha256) {
 if (cnt.indexOf("over-tlstrue")!-1 cnt.indexOf("obfswss")!-1) {
  cnt cnt.replace(/tls-verification\s*\\s*false(\s*)\,/,"") //去除 tls-verificationfalse
  //notify("SHA256",cnt,Pcsha256Ppsha256)
...
function ALPN_Handle(cnt,palpn) {
 ...
 if (cnti.indexOf("obfsover-tls") ! -1 cnti.indexOf("over-tlstrue")!-1) {
...
function Mock2QXReject(row, filename) {
  ...
  } else if (/(txthtml)/i.test(filename)) {
    return row.replace(/ /g, "").split("data")[0] " url " "reject-200"
  } else if (/(pngjpggif)/i.test(filename)) {
 ...
 for (var i 0; i subs.length; i) {
  try {
   ...
   if (!NoteK.some(notecheck) && !RewriteK.some(RewriteCheck)){
    if(Pdbg1) {notify("rewrite-check","",subs[i])}
    if (SC.every(sccheck)) { // surge js 新格式
     ...
     subsi subs[i].replace(/ /g,"").replace(/\true/g,"1")
     if (type "http-response" && subsi.indexOf("requires-body1") ! -1) {
      type "script-response-body "
     } else if (type "http-response" && subsi.indexOf("requires-body1") -1) {
      type "script-response-header "
     } else if (type "http-request" && subsi.indexOf("requires-body1") ! -1) {
    ...
    } else if (/\s30(72)/.test(subs[i])) { //rewrite 302&307 复写(Surge)
     ...
     nrw.push(rw)
    } else if (/\s\-\s30(27)\s/.test(subs[i])) { //rewrite 302&307 复写(Shadowrocket)
     //xx - 302 123
     ...
     //rw subs[i].split(" ")[0] " url request-header GET " pget "(.\\r\\n)Host:.(\\r\\n) request-header GET " pgetn "1Host: " subs[i].split(" ")[1].split("://")[1].split(".com")[0] ".com2"
     nrw.push(rw)
...
//Rewrite过滤，使用连接多个关键词(逻辑"或"):in 为保留，out 为排除
...
function Rule_Handle(subs, Pout, Pin) {
 ...
 nlistnlist.map(itemitem.replace(/:\d*\s*,/g,",").replace(/(\'\")/g,"").replace(/(\-suffix\-SUFFIX)\s*\,\s*\./g,"1, ")) //去除端口号以及分号部分, 以及部分suffix规则以. 开头的问题
 //notify("nlist","",nlist)
 return nlist
}
...
function rule_list_handle(cnt) {
 ...
 if (cnt.trim().indexOf(" ") -1 && cnt.trim() ! "" && !RuleK.some(RuleCheck)) {
  if (cnt.indexOf("::") ! -1 && cnt.indexOf("/") ! -1) { // ip-v6?
   ...
   //notify("xxx","xxxx",cnt)
   //cntcnt.replace(/'"/g,"").trim()//replace(/'"\\.\*\.\*\.\*/g,"") 2023-04-10
...
// filter 正则指定替换 regex1@policy1regex2@policy2
function policy_sets(cnt,para) {
 pcnt para.split("")
 cntcnt//.split("\n")
 for (i0;ipcnt.length;i){
  console.log(pcnt[i])
  if (pcnt[i].indexOf("@")!-1){
   cnt cnt.map(item filter_set(item, pcnt[i]))
  }
 }
 cntcnt.filter(Boolean)//.join("\n")
 return cnt
 console.log(cnt)
}
 ...
 if (Pdbg) {notify("subs", "node", subs)}
  ...
  for (var i 0; i list0.length; i) {
    ...
    if (list0[i].trim().length 3 && !/\;\/\#/.test(list0[i][0]) && list0[i].indexOf(" url ")-1) {
      ...
      var tag0 list0[i].indexOf("tag")!-1 ? list0[i].split(/\&*(emojiudptfocertrenamereplace)\/)[0].split("tag")[1] : ""
      ...
      try {
       if (Pdbg) {notify(i, type, list0[i])}
        if (type "vmess" && (list0[i].indexOf("remark") -1 && list0[i].indexOf("remarks") -1) && !/(obfsalterIdtype\@)\/.test(list0[i])) {
        ...
        } else if (type "vmess" && ( list0[i].indexOf("remark") ! -1 list0[i].indexOf("remarks") ! -1 /(obfsalterIdtype\@)\/.test(list0[i]))) { //shadowrocket 类型
        ...
        } else if (type "vless" && version821) {
         Perror 1 ; // 无需反馈
         notify("️ 你的 Quantumult X 版本暂未支持 Vless 节点","请 ️不要️ 跑来 解析器️ 反馈",list0[i])
        } else if (type "vless" ) { // version 150 support vless 
         nodeVL2QX(list0[i], Pudp, Ptfo, Pcert0, PTls13)
...
function AeadVmess(cnt,aeadp) {
 let paead aeadp -1? "aeadfalse" : "aeadtrue" 
 if (/vmess\s*\/.test(cnt)) {
  if (/aead\s*\/.test(cnt)) {
...
function TLS_Check(cnt) {
 cnt cnt.indexOf("tls-cert-sha256")!-1 cnt.indexOf("tls-pubkey-sha256")!-1 ? cnt.replace(/tls-verification\s*\\s*false.*?\,/,"tls-verificationtrue,"): cnt // 去掉 tls-verificationfalse 如果存在 sha256
 return cnt
}
...
function QX_TLS(cnt,Pcert0,PTls13) {
 ...
 var cert0 Pcert0 1? "tls-verificationtrue, " : "tls-verificationfalse, "
 var tls13 PTls13 1? "tls13true, " : ""
 ...
 }else if(cnt.indexOf("obfsover-tls")!-1 /over\-tls\s*\\s*true/.test(cnt) cnt.indexOf("obfswss")!-1){ //未包含tls参数时
  cnt cnt.replace(new RegExp("tag.*?\", "gmi"), cert0"tag")
 }
 if (tls13 !"") {
 if(cnt.indexOf("tls13") ! -1){
  cnt cnt.replace(RegExp("tls13.*?\,", "gmi"), tls13)
 }else if(cnt.indexOf("obfsover-tls")!-1 /over\-tls\s*\\s*true/.test(cnt) cnt.indexOf("obfswss")!-1){
  cnt cnt.replace(new RegExp("tag.*?\", "gmi"), tls13"tag")
 }
 }
 if (!/(shadowsockstrojanvmess)/.test(cnt.trim())) { //关闭非 ss/ssr/trojan/vmess 类型的 udp
  udp  "udp-relayfalse, "
...
//httpexample.com:443, usernamename, passwordpwd, over-tlstrue, tls-hostexample.com, tls-verificationtrue, tls13true, fast-openfalse, udp-relayfalse, taghttp-tls-02
...
function HPS2QX(subs, Ptfo, Pcert0, PTls13) {
  ...
  if (server ! "") {
   if (server.indexOf("@")!-1) {
    ...
    var tls type "https"? "over-tlstrue": "";
    var thost subs.indexOf("peer")! -1? "tls-host" subs.split("peer")[1].split("#")[0].split("&")[0] : "" // 存在peers参数时 https://b64(ipport)?peerxxx#server-remarks
    var cert Pcert0 ! 0 ? "tls-verificationtrue" : "tls-verificationfalse";
    var tfo Ptfo 1 ? "fast-opentrue" : "fast-openfalse";
    var tls13 PTls13 1 ? "tls13true" : "tls13false";
    if (tls"") {
     cert""
     tls13""
    }
...
function VQ2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
 ...
 var node ""
 var ip "vmess" server.split(",")[1].trim() ":" server.split(",")[2].trim() ", " "methodaes-128-gcm, " "password" server.split(",")[4].split("\"")[1] ", "
 var tag "tag" server.split("")[0]
 var tfo subs.indexOf("tfo1") ! -1 ? "fast-opentrue, " : "fast-openfalse, "
 var udp Pudp 1 ? "udp-relayfalse, " : "udp-relayfalse, "; // 不支持 vmess 类型 udp
 node ip tfo udp
 var obfs ""
 if (server.indexOf("obfs") -1) { // 非 ws/http 类型
  obfs server.indexOf("over-tlstrue") ! -1 ? "obfsover-tls, " : "" //over-tls
  var host server.indexOf("tls-host") ! -1 ? "obfs-host" server.split("tls-host")[1].split(",")[0] ", " : ""
  obfs obfs host
 } else if (server.indexOf("obfsws") ! -1) {
  obfs server.indexOf("over-tlstrue") ! -1 ? "obfswss, " : "obfsws, " //ws,wss 类型
  ...
  obfs obfs host
 } else if (server.indexOf("obfshttp") ! -1) {
  obfs "obfshttp, "
  var uri server.indexOf("obfs-path") ! -1 ? "obfs-uri" server.split("obfs-path")[1].split("\"")[1] ", " : "obfs-uri/, "
  obfs obfs uri
  var host server.indexOf("obfs-header") ! -1 ? "obfs-host" server.split("obfs-header\"Host:")[1].split("[")[0].trim() ", " : ""
 ...
 if (obfs.indexOf("obfsover-tls") ! -1 obfs.indexOf("obfswss") ! -1) {
  var cert Pcert0 ! 0 subs.indexOf("allowInsecure1") ! -1 ? "tls-verificationfalse, " : "tls-verificationtrue, "
  var tls13 PTls13 1 ? "tls13true, " : ""
...
function VR2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
 ...
 if(Pdbg1) {notify("Shadowrocket-Vmess-URI","..",subs"\n\n"server)}
 if (server.indexOf("@")-1 && subs.indexOf("@")!-1) { server subs.replace("vmess://", "").split("?")[0]}
 var node ""
 var ip "vmess" server.split("@")[1] ", " "methodaes-128-gcm, " 
 ...
 var tfo subs.indexOf("tfo1") ! -1 ? "fast-opentrue, " : "fast-openfalse, "
 var udp Pudp 1 ? "udp-relayfalse, " : "udp-relayfalse, ";
 var pdrop 0
 node ip pwd tfo udp
 var obfs subs.indexOf("obfs")!-1 ? subs.split("obfs")[1].split("&")[0].trim() : "none"
 if (obfs "none") { //
  obfs subs.indexOf("tls1") ! -1 ? "obfsover-tls, " : "" //over-tls
 } else if (obfs "websocket" obfs "http") {
  obfs obfs "http" ? "obfshttp, " : "obfsws, " // http 类型
  obfs subs.indexOf("tls1") ! -1 ? "obfswss, " : obfs //ws,wss 类型
  ...
  Perror 1 // 不需要反馈的类型
  if (Pntf0!0) {
  notify( "️ Quantumult X 暂不支持该类型节点", "已忽略以下 grpch2 vmess 节点",subs)
 }
  pdrop 1
 }
 if (obfs.indexOf("obfsover-tls") ! -1 obfs.indexOf("obfswss") ! -1) {
 ...
 if (subs.indexOf("alterId") ! -1) {
  caead Number(subs.split("alterId")[1].split("&")[0]) ! 0 ? "aeadfalse, " : ""
 }
 node pdrop0? node obfs caead tag : ""
 //notify(node)
 return node
}
...
function S5R2QX(cnt,tlsp"false") {
 ...
 if (server ! "") {
   ...
   var tls tlsp"false"? "":"over-tlstrue"
   var cert Pcert0 ! 0 ? "tls-verificationtrue" : "tls-verificationfalse";
   cert tls ""? "":cert
   var tfo Ptfo0 1 ? "fast-opentrue" : "fast-openfalse";
...
function V2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
 ...
 if (server ! "") {
  ...
  pwd "password" ss.id;
  mtd "methodaes-128-gcm"
  try {
   tag "tag" decodeURIComponent(ss.ps);
  } catch (e) {
   tag "tag" ss.ps;
  }
  udp Pudp 1 ? "udp-relayfalse" : "udp-relayfalse";
  tfo Ptfo 1 ? "fast-opentrue" : "fast-openfalse";
  obfs Fobfs(ss, cert, tls13);
  caead ss.aid && ss.aid ! "0" ? "aeadfalse" : "aeadtrue"; //aead 选项
...
function Fobfs(jsonl, Pcert0, PTls13) {
 var obfsi [];
 var cert Pcert0;
 tcert cert 0 ? "tls-verificationfalse" : "tls-verificationtrue";
 tls13 PTls13 1 ? "tls13true" : "tls13false"
 if (jsonl.net "ws" && jsonl.tls "tls") {
  obfs0 "obfswss, " tcert ", " tls13 ", ";
  ...
  obfs0 "obfsover-tls, " tcert ", " tls13;
...
//节点过滤，使用连接多个关键词(逻辑"或"):in 为保留，out 为排除, "与"逻辑请用符号"."连接
...
//a.c.com:0031:origin:aes-256-gcm:plain:pwdpwd/?obfsparam&remarksxxxx
...
function SSR2QX(subs, Pudp, Ptfo) {
  ...
  var obfshost '';
  var oparam '';
  if(Pdbg1) {notify("ssr","content",cnt)}
  if (cnt.split(":").length 8) { //排除难搞的 ipv6 节点
    ...
    pudp Pudp 1 ? "udp-relaytrue" : "udp-relayfalse";
    ptfo Ptfo 1 ? "fast-opentrue" : "fast-openfalse";
...
// vless://pwd@a.b.c.gq:443?encryptionnone&securitytls&typews&hosta.b.c.d&pathdsjdaaaaj#VLESS_WSS
// Vless Shadowrocket URI
// vless://YXV0bzpkampkakAxLjEuMS4xOjY2NjY?remarksvless&obfsParam123.com&path/jsjdj&obfswebsocket&tls1&peerabc.com&tfo1
//;vlessexample.com:443, methodnone, password23ad6b10-8d1a-40f7-8ad0-e3e35cd32291, obfswss, obfs-uri/ws, fast-openfalse, udp-relayfalse, tagvless-ws-tls-01
//vless://YXV0bzpkampkakAxLjEuMS4xOjY2NjY?remarksvless&obfsParamhshdh&path/jsjdj&obfshttp&tls1&peerabc.com&tfo1
//vls VLESS,1.1.1.1,443,"b0dd64e4-0fbd-4038-9139-d1f32a68a0dc",transportws,pathpatha,hosthost.com,udptrue,over-tlstrue,tls-namesni.co
function VL2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
 ...
 pcert cnt.indexOf("allowInsecure0") ! -1 ? "tls-verificationtrue" : "tls-verificationfalse";
 ...
 pudp (Pudp 1 cnt.indexOf("udp1")!-1) ? "udp-relaytrue" : "udp-relayfalse";
 ptfo (Ptfo 1 cnt.indexOf("tfo1")!-1)? "fast-opentrue" : "fast-openfalse";
 //ptfo cnt.indexOf("tfo1") ! -1? "fast-opentrue" : ptfo
 if (typeU "SR-URI") {//小火箭内的websocket写法
  if(cnt.indexOf("obfsnone")!-1 && cnt.indexOf("tls1")-1) {
   obfs ""
  } else if(cnt.indexOf("obfsnone")!-1 && cnt.indexOf("tls1")!-1) {
   obfs "obfsover-tls"
  } else if(cnt.indexOf("obfshttp")!-1) {
   obfs "obfshttp"
  } else if(cnt.indexOf("obfswebsocket")!-1) {
   obfs cnt.indexOf("tls1") ! -1? "obfswss" : "obfsws"
  } 
 thostcnt.indexOf("obfsParam") -1? thost : "obfs-host" decodeURIComponent(cnt.split("obfsParam")[1].split("&")[0].split("#")[0])
 puri cnt.indexOf("path") -1? puri : "obfs-uri" decodeURIComponent(cnt.split("path")[1].split("&")[0].split("#")[0])
 } else if (cnt.indexOf("&typews")!-1 cnt.indexOf("?typews")!-1 cnt.indexOf("typehttp")!-1 cnt.indexOf("securitytls")!-1) {//v2rayN uri
  if(cnt.indexOf("typehttp") ! -1) {
   obfs"obfshttp"
  } else if (cnt.indexOf("typews") ! -1) {
   obfs cnt.indexOf("securitytls") ! -1? "obfswss" : "obfsws" 
  } else if(cnt.indexOf("securitytls")!-1) {
   obfs "obfsover-tls"
  }
  thostcnt.indexOf("&host") -1? thost : "obfs-host" decodeURIComponent(cnt.split("&host")[1].split("&")[0].split("#")[0])
  puri cnt.indexOf("&path") -1? puri : "obfs-uri" decodeURIComponent(cnt.split("&path")[1].split("&")[0].split("#")[0])
 } else if(cnt.indexOf("securityxtls")!-1) { //暂不支持类型
  type"NS"
 } else if(cnt.indexOf("type")!-1 && cnt.indexOf("typetcp")-1) {//暂不支持类型
  type"NS"
 }
if(obfs"obfswss" && obfs"obfsover-tls"){
 ptls13 PTls13 1 ? "tls13true" : "tls13false"
 if (Pcert0 0) { 
  pcert "tls-verificationfalse" 
 } else if (Pcert0 1) {
  pcert "tls-verificationtrue"
 }
} else {
 pcert""
 ptls13""
...
function TJ2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  ...
  if (cnt.indexOf("&pluginobfs-local")!-1) {//小火箭内的websocket写法
  obfs cnt.indexOf("obfswebsocket") ! -1? "obfswss" : obfs 
  thostcnt.indexOf("obfs-host") -1? thost : "obfs-host" decodeURIComponent(cnt.split("obfs-host")[1].split(";")[0].split("#")[0])
  puri cnt.indexOf("obfs-uri") -1? puri : "obfs-uri" decodeURIComponent(cnt.split("obfs-uri")[1].split(";")[0].split("#")[0])
  } else if (cnt.indexOf("&typews")!-1 cnt.indexOf("?typews")!-1) {//v2rayN uri
   obfs cnt.indexOf("securitytls") ! -1? "obfswss" : obfs 
   thostcnt.indexOf("&host") -1? thost : "obfs-host" decodeURIComponent(cnt.split("&host")[1].split("&")[0].split("#")[0])
   puri cnt.indexOf("&path") -1? puri : "obfs-uri" decodeURIComponent(cnt.split("&path")[1].split("&")[0].split("#")[0])
  }
...
function SS2QX(subs, Pudp, Ptfo) {
 ...
 if (cnt.split(":").length 10) { //排除难搞的 ipv6 节点
  ...
  if (cntt.indexOf("v2ray-plugin")-1 && cntt.indexOf("pluginv2ray")-1) { //Shadowrocket style v2-plugin
   ...
   obfs cnt1.tls? ", obfswss" : ", obfsws"
   obfshost cnt1.host? ", obfs-host"cnt1.host", tls-verificationfalse" : ""
  } else if (cntt.indexOf("v2ray-plugin")!-1){
   cnt1 decodeURIComponent(cntt.split("v2ray-plugin")[1])
   obfs cnt1.indexOf("tls")!-1? ", obfswss" : ", obfsws"
   obfshost cnt1.indexOf("host")!-1? ", obfs-host"cnt1.split("host")[1].split(";")[0].split("#")[0].trim() : ""
   obfshost obfshost ! "obfs-host"? obfshost : ""
   //notify("CNTT","",cnt1obfsobfshost)
  } else if (cntt.indexOf("pluginv2ray")!-1) {
   cnt1 decodeURIComponent(cntt.split("pluginv2ray")[1])
   obfs cnt1.indexOf("tls")!-1? ", obfswss" : ", obfsws"
   obfshost cnt1.indexOf("host")!-1? ", obfs-host"cnt1.split("host")[1].split(";")[0].split("#")[0].trim() : ""
   obfshost obfshost ! "obfs-host"? obfshost : ""
   //notify("CNTT","",cnt1obfsobfshost)
  ...
  QX nssr.join(", ")
  if(Pdbg1) {notify("SS","content",cnt"\n"QX)}
 }
 return QX;
}
...
try {
 ...
 cnti tail.length1? cnti : String(hd ","tail[1].split("\r")[0] "," tag)
 ...
 for (i in cntis) {
  if (cntis[i].indexOf("") -1 && cntis[i].trim() !"") { // tag 中多出的项目
   ...
   cntis[i].indexOf("tag") ! 0? cntii cntis[i]", ": cntiicntii
  }
 }
 cntii cntiitagtagfix
 ...
 for (var i 0; i cnt.length; i){
  if(cnt[i]){
   ...
   if (cnti.indexOf("pattern")!-1 && cnti.indexOf("type")!-1 cnti.indexOf("http-r")!-1) {
    cntiSGMD2QX(cnti)[0]? SGMD2QX(cnti)[0]:""
    //console.log(cnti)
   }else if ((cnti.indexOf(" 302")!-1 cnti.indexOf(" 307")!-1 (/\s(_-)\s(rejectREJECT)/.test(cnti)) (/\sreject/.test(cnti)) (/\sreject-/.test(cnti))) && cnti.indexOf(" url ")-1 && cnti.indexOf(" url-and-header ")-1 ){
    cntiSGMD2QX(cnti)[0]? SGMD2QX(cnti)[0]:""
    //console.log("sss",cnti)
   }else if(cnti.indexOf(" data")!-1){
    cnti SGMD2QX("[Map Local]\n"cnti)[0]? SGMD2QX("[Map Local]\n"cnti)[0]:""
    //cnticnti.replace(/ /g, "").split("data")[0] " url " "reject-dict"
   ...
   if (cnti!"" && cnti.trim()[0]!"[" && cnti.indexOf("RULE-SET")-1 && !/cronexp\type\cron/.test(cnti.replace(/ /g,"")) && !RuleK.some(RuleCheck)) {
...
function Sort_KWD (cnt,strs) {
 ...
 var arr  new Array(strlist.length1);  //表格有n行
 for(var i 0;i arr.length; i){
  arr[i] [];  //每行有列
 }
 for (var i 0; icnt.length ; i) {
  flag 0
  for (var j0; jstrlist.length ; j){
...
function DelReg(content) {
  ...
  contentcontent.replace(/tag\s*\\s*/,"tag")
  cnt0 content.split("tag")[0]
  cnt1 content.split("tag")[1].split(",")[0]
  cnt2 content.split("tag")[1].split(",").length1 ? "": content.split(cnt1)[1]
...
// 俄罗斯 GIA custom, ip, 152, aes-128-gcm, password123, https://xxx/download/SSEncrypt.module, obfstls, obfs-hostxxx.windows.com, udp-relaytrue
function SCT2QX(content) {
  ...
  var ptfo paraCheck(cnt, "tfo") "true" ? "fast-opentrue" : "fast-openfalse";
  var pudp paraCheck(cnt, "udp-relay") "true" ? "udp-relaytrue" : "udp-relayfalse";
...
function SVmess2QX(content) {
  ...
  var pmtd "methodaes-128-gcm";
  var ptls13 paraCheck(cnt, "tls13") "true" ? "tls13true" : "tls13false";
  var pverify cnt.replace(/ /g,"").indexOf("skip-cert-verifyfalse") ! -1 ? "tls-verificationtrue" : "tls-verificationfalse";
  pvefify Pcert0 1? "tls-verificationtrue" : pverify ;
  if (paraCheck(cnt.replace(/tls13/, ""), "tls") "true" && paraCheck(cnt.replace(/ws-header/, ""), "ws") "true") {
    pobfs "obfswss" ", " ptls13 ", " pverify
  } else if (paraCheck(cnt.replace(/ws-header/, ""), "ws") "true") {
    pobfs "obfsws"
  } else if (paraCheck(cnt.replace(/tls13/, ""), "tls") ! "false") {
    pobfs "obfsover-tls" ", " ptls13 ", " pverify
...
function Shttp2QX(content) {
 ...
 var ptls cnt.split("")[1].split(",")[0].trim() "https" ? "over-tlstrue" : "over-tlsfalse";
 var ptfo paraCheck(cnt, "tfo") "true" ? "fast-opentrue" : "fast-openfalse";
 if (ptls "over-tlstrue") {
  var pverify cnt.replace(/ /g,"").indexOf("skip-cert-verifyfalse") ! -1 ? "tls-verificationtrue" : "tls-verificationfalse";
  pvefify Pcert0 1? "tls-verificationtrue" : pverify ;
  var ptls13 paraCheck(cnt, "tls13") "true" ? "tls13true" : "tls13false";
...
function SS52QX(content) {
 ...
 var puname cnt.indexOf("username") ! -1 ? "username" cnt.split("username")[1].split(",")[0].split("")[1].trim() : "";
 var pwd cnt.indexOf("password") ! -1 ? "password" cnt.split("password")[1].split(",")[0].split("")[1].trim() : "";
 var ptls cnt.split("")[1].split(",")[0].trim() "socks5-tls" ? "over-tlstrue" : "over-tlsfalse";
 var ptfo paraCheck(cnt, "tfo") "true" ? "fast-opentrue" : "fast-openfalse";
 if (ptls "over-tlstrue") {
  var pverify cnt.replace(/ /g,"").indexOf("skip-cert-verifyfalse") ! -1 ? "tls-verificationtrue" : "tls-verificationfalse";
  pvefify Pcert0 1? "tls-verificationtrue" : pverify ;
...
//Loon 的 ssr 部分
//# SSR 格式：名称协议类型,地址,端口,加密方式,密码,协议类型,{协议参数},混淆类型,{混淆参数}
...
function LoonVL2QX(cnt) {
 var tag ", tag" cnt.split("")[0].trim()
 cntcnt.replace(" ","") //去掉空格 简化 
 var node "vless"
 var ip [cnt.split(",")[1].trim(), cnt.split(",")[2].trim()].join(":")
 var mtd "methodnone"
 var pwd "password" cnt.split(",")[3].trim().split("\"")[1]
 if (cnt.indexOf("transporttcp")!-1) {
  obfs cnt.indexOf("over-tlstrue")"-1"? "":"obfsover-tls"
 } else if (cnt.indexOf("transporthttp")!-1) {
  obfs"obfshttp"
 } else if (cnt.indexOf("transportws")!-1) {
  obfs cnt.indexOf("over-tlstrue")"-1"? "obfsws":"obfswss"
 }
...
function YAMLFix(cnt){
 ...
 if (cnt.indexOf("{") ! -1 && /\{\s*\"*(nametypeserver)/.test(cnt)){
  cnt cnt.replace(/(\n)- /g, "1 - ").replace(/  - /g," - ").replace(/:(?!\s)/g,": ").replace(/\,\"/g,", \"").replace(/: {/g, ": {,  ").replace(/, (Hosthostpathmux)/g,",  1")
  //2022-04-11 remove tlsskip from replace(/, (Hosthostpathmux)/g,",  1")
  console.log("1st:\n"cnt)
  cnt cnt.replace(/{\s*name: (.*?), (.*?):/g,"{name: \"1\", 2:") //cnt.replace(/{\s*name: /g,"{name: \"").replace(/, (.*?):/,"\", 1:")
  cnt cnt.replace(/{\s*\s*}/g,"").replace(/,/g,"\n  ")
 }
 cnt cnt.replace(/\n\s*\-\s*\n.*name/g,"\n - name").replace(/\\/g,"").split("proxy-providers:")[0].split("proxy-groups:")[0].replace(/\"(nametypeserverportcipherpassworduuidalterIdudp)(\"*)/g,"1")
  if(Pdbg 1) {
 ...
 //2022-09-01 remove host in s{6}(Hh)ost
...
function Clash2QX(cnt) {
 const yaml new YAML()
 if (Pdbg1) { notify(" Before YAML Parse", "content", cnt)}
 ...
 if (Pdbg1) { notify("After YAML Parse", "content", JSON.stringify(bb))}
 //console.log(bb)
 var nl bb.length
 var nodelist[]
 var node""
...
function CSS2QX(cnt) {
 ...
 udp cnt.udp ? "udp-relaytrue" : "udp-relayfalse"
 uot cnt["udp-over-tcp"] ? "udp-over-tcptrue" : "udp-over-tcpfalse"
 tfo cnt.tfo ? "fast-opentrue" : "fast-openfalse"
...
function CV2QX(cnt) {
 ...
 udp cnt.udp ? "udp-relayfalse" : "udp-relayfalse" //暂不支持
 tfo cnt.tfo ? "fast-opentrue" : "fast-openfalse"
 obfs ""
 ...
 cert cnt["skip-cert-verify"] && cnt.tls ? "tls-verificationfalse" : ""
 caead cnt["alterId"] && cnt["alterId"]!0? "aeadfalse" : "" // aead 选项
 //caead cnt["alterId"] 0? "aeadtrue" : caead // aead 选项
 console.log(caead)
 //caead""
 //notify(cert)
 if (Pcert0 1 && cnt.tls) {
  cert "tls-verificationtrue"
 } else if (Pcert0 ! 1 && cnt.tls) {
  cert "tls-verificationfalse"
 }
...
function CT2QX(cnt) {
 ...
 opath""
 ohost""
 cert cnt["skip-cert-verify"] ? "tls-verificationfalse" : "tls-verificationtrue"
 cert Pcert0 1 ? "tls-verificationtrue" : "tls-verificationfalse"
 tls13 PTls13 1 ? "tls13true" : "tls13false"
 udp cnt.udp ? "udp-relayfalse" : "udp-relayfalse"
 tfo cnt.tfo ? "fast-opentrue" : "fast-openfalse"
...
function XUDP(cnt,pudp) {
 var udp pudp 1 && /(shadowsockstrojanvmess)/.test(cnt.trim()) ? "udp-relaytrue, " : "udp-relayfalse, "
 if(cnt.indexOf("udp-relay") ! -1){
...
function UOT(cnt) {
 ...
 if(/shadowsocks/.test(cnts)) {
  cnt cnts.indexOf("udp-over-tcp")!-1? cnt.replace(/udp-over-tcp\s*\\s*false/g,"udp-over-tcptrue") : cnt", udp-over-tcptrue"
...
function Tools() {
  ...
  const getNodeInfo servers {
    const nodes {
      ...
      types: servers.map(s {
        const type s.match(/(vmesstrojanshadowsockshttp)/);
